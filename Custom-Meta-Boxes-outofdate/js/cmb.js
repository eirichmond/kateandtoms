...
var CMB = {
	...
	init : function() {
		...
		jQuery('.field-item > .cmb_metabox').after('<button class="cmb-insert-field"><span class="dashicons dashicons-plus"></span></button>');
...
*/
		...
		jQuery(document).on( 'click.CMB', '.cmb-insert-field', CMB.duplicateField );
	...
	duplicateField : function( e ) {
		...
		newT.insertAfter( jQuery(this).parent() );
	...
	clonedField: function( el ) {
		...
		el.add( el.find( 'div[data-class]' ) ).each( function( i, el ) {
			...
			jQuery('#special-offers .field-item input').unbind('keyup change input paste').bind('keyup change input paste',function(el){
