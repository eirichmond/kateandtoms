jQuery('#searchBar input[type="hidden"]').on('change', function(){ // this i happening twice and shouldn't probably need to refactor
	//debugger;

	updateDisplayedMetaData(this);
	var path = window.location.href;
	var isHousesSearch = path.includes('houses');
	if(window.location.search === '') { // check this is the home page
		if(!isHousesSearch) {
...
//		1 : { 5 : +1, 6 : -0, 0 : -2, 1 : +5, 2 : +4, 3 : +3, 4 : +2 }, // Weekends
...
jQuery(function() {
	jQuery('#msbhslide').on('touchstart', function(e) {
		e.preventDefault();
		jQuery('#msbh').slideToggle();
		jQuery('.ktsub_nav_cont').toggleClass('toggled');
		jQuery('#mobilenav').css('height', '0px');
	});
	jQuery('.close.house-search').on('touchstart', function() {
		jQuery('#msbh').slideToggle();
		jQuery('.ktsub_nav_cont').toggleClass('toggled');

	});
	jQuery('#msbdslide').on('touchstart', function(e) {
		e.preventDefault();
		jQuery('#header #searchBar').slideToggle();
		jQuery('.ktsub_nav_cont').toggleClass('toggled');
		jQuery('#mobilenav').css('height', '0px');
